...
export async function clientSaveFieldToPostgres(field: string, value: any) {
  try {
    if (field === "areas") {
      await runClientQuery("DELETE FROM sia_areas");
      ...
      await runClientQuery("DELETE FROM sia_licenses");
      if (Array.isArray(value)) {
      ...
      await runClientQuery("DELETE FROM sia_component_types");
      if (Array.isArray(value)) {
      ...
      await runClientQuery("DELETE FROM sia_inventory_items");
      if (Array.isArray(value)) {
      ...
      await runClientQuery("DELETE FROM sia_decommissioned_items");
      if (Array.isArray(value)) {
      ...
      await runClientQuery("DELETE FROM sia_audit_logs");
      if (Array.isArray(value)) {
      ...
      await runClientQuery("DELETE FROM sia_assets");
    ...
    console.error(`[Postgres Client] Error seeding/saving field [${field}]:`, err);
    throw err;
  }
}
...
export async function clientMigrateAllToPostgres(localData: any) {
  ...
  for (const field of fields) {
    if (localData[field] !== undefined) {
      await clientSaveFieldToPostgres(field, localData[field]);
    }
  }
}
...
export async function clientLoadAllFromPostgres() {
  try {
    const [areasRows, licsRows, compsRows, invRows, decRows, assetsRows, logsRows] = await Promise.all([
      runClientQuery("SELECT * FROM sia_areas"),
      runClientQuery("SELECT * FROM sia_licenses"),
      runClientQuery("SELECT * FROM sia_component_types"),
      runClientQuery("SELECT * FROM sia_inventory_items"),
      runClientQuery("SELECT * FROM sia_decommissioned_items"),
      runClientQuery("SELECT * FROM sia_assets"),
      runClientQuery("SELECT * FROM sia_audit_logs ORDER BY timestamp DESC LIMIT 500"),
    ]);
